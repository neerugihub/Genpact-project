...
public class WithdrawServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            stmt1 = conn.prepareStatement("UPDATE Customer SET initial_balance = initial_balance - ? WHERE account_no=? AND initial_balance >= ?");
            stmt1.setBigDecimal(1, amount);
            stmt1.setString(2, accountNo);
            stmt1.setBigDecimal(3, amount);
            int rows = stmt1.executeUpdate();
            if (rows > 0) {
                stmt2 = conn.prepareStatement("INSERT INTO Transaction (account_no, transaction_type, amount) VALUES (?, 'Withdraw', ?)");
